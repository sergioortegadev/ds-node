...
const analizeGame = () => {
  ...
  if (points[0] > 5 || points[1] > 5) {
    ...
    return analizeSet(); //Llamo function para analizar si con este game tenemos ganador del partido. Me retornará true o false.
  }
...
const analizeSet = () => {
  const gamesWinP1 = games.filter((el) => {
    return el === "Player 1";
  });

  const gamesWinP2 = games.filter((el) => {
    return el === "Player 2";
  });
